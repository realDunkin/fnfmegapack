...
class FreeplayWhitty extends MusicBeatState
{
	...
	override function create()
	{
		...
		var initSonglist = CoolUtil.coolTextFile(Paths.txt('whittylist'));
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin && selectable)
		{
			...
			if (controls.ONE)
			{
				...
				var poop:String = Highscore.formatSong("lo-mongus".toLowerCase(), curDifficulty);
				trace(poop);
				...
				PlayState.SONG = Song.loadFromJson(poop, "lo-mongus".toLowerCase());
	...
	function changeDiff(change:Int = 0)
	{
		...
		rank.loadGraphic(Paths.image('rankings/'+rankTable[Highscore.getRank(songs[curSelected].songName.toLowerCase(), curDifficulty)]));
		rank.setGraphicSize(0,90);
		rank.updateHitbox();
		rank.scrollFactor.set();
		rank.y = 690 - rank.height;
