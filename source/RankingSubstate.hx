...
class RankingSubstate extends MusicBeatSubstate
{
	...
	public static var inrankingsubstate:Bool = false;
	...
	public function new(x:Float, y:Float)
	{
		...
		{
			Highscore.saveRank(PlayState.SONG.song.toLowerCase(), rankingNum, PlayState.storyDifficulty);
		}
		MenuMusic.RankMusic();
		pauseMusic.volume = 0;
		...
		{
			hint.y -= 35;
			hint.text = "If you wanna gather that rank, don't enable botplay.";
		}
	...
	override function update(elapsed:Float)
	{
		...
		var pressedAnything:Bool = FlxG.keys.justPressed.ANY;
		...
		if (pressedAnything || _modifiers.Practice)
		{
			PlayState.ended = false;
			inrankingsubstate = false;
			...
			switch (PlayState.gameplayArea)
			{
				case "Story":
					if (PlayState.storyPlaylist.length <= 0)
					{
						...
						{
							case 'hellclown':
								LoadingState.loadAndSwitchState(new VideoState("assets/videos/tricky/TricksterMan.webm",new MainMenuState()));
							default:
