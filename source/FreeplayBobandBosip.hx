...
import MenuCategorys.SongMetadata;

using StringTools;

class FreeplayBobandBosip extends MusicBeatState
{
	...
	override function create()
	{
		...
		var initSonglist = CoolUtil.coolTextFile(Paths.txt('bobandbosiplist'));
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin && selectable)
		{
			...
			if (accepted)
			{
				...
				var poop:String = Highscore.formatSong(songs[curSelected].songName.toLowerCase(), difficulties[curDifficulty]);
				trace(poop);
	...
	function updateDifficultyList():Void
	{
		...
		if (Main.exMode && songs[curSelected].songCharacter == 'bob')
		{
			songs[curSelected].songCharacter = 'bobex';
		}
		if (!Main.exMode && songs[curSelected].songCharacter == 'bobex')
		{
			songs[curSelected].songCharacter = 'bob';
		}
		if (Main.exMode && songs[curSelected].songCharacter == 'bosip')
		{
			songs[curSelected].songCharacter = 'bosipex';
		}
		if (!Main.exMode && songs[curSelected].songCharacter == 'bosipex')
		{
			songs[curSelected].songCharacter = 'bosip';
		}
		if (Main.exMode && songs[curSelected].songCharacter == 'amor')
		{
			songs[curSelected].songCharacter = 'amorex';
		}
		if (!Main.exMode && songs[curSelected].songCharacter == 'amorex')
		{
			songs[curSelected].songCharacter = 'amor';
		}
		if (Main.exMode && songs[curSelected].songCharacter == 'gf')
		{
			songs[curSelected].songCharacter = 'gf-ex';
		}
		if (!Main.exMode && songs[curSelected].songCharacter == 'gf-ex')
		{
			songs[curSelected].songCharacter = 'gf';
		}
	}
	...
	function changeDiff(change:Int = 0)
	{
		...
		#if !switch
		intendedScore = Highscore.getScore(songs[curSelected].songName.toLowerCase(), difficulties[curDifficulty]);
		rank.loadGraphic(Paths.image('rankings/'+rankTable[Highscore.getRank(songs[curSelected].songName.toLowerCase(), difficulties[curDifficulty])]));
		rank.setGraphicSize(0,90);
		rank.updateHitbox();
		rank.scrollFactor.set();
		rank.y = 690 - rank.height;
		...
		if (songs[curSelected].songName.toLowerCase() == 'tutorial-bnb' || songs[curSelected].songName.toLowerCase() == 'jump-in' || songs[curSelected].songName.toLowerCase() == 'swing' || songs[curSelected].songName.toLowerCase() == 'split')
		{
			if (controls.LEFT_P)
			{
				if (difficulties[curDifficulty] == 2)
				{
					#if PRELOAD_ALL
					vocals.stop();
					#end
				...
				if (difficulties[curDifficulty] == 3)
				{
					#if PRELOAD_ALL
					vocals.stop();
					#end
			...
			{	
				...
				if (difficulties[curDifficulty] == 0)
				{
					#if PRELOAD_ALL
					vocals.stop();
					#end
	...
	function changeSelection(change:Int = 0)
	{
		...
		#if desktop
		PlayState.iconRPC = songs[curSelected].songCharacter;
		...
		disc.animation.addByPrefix(songs[curSelected].songCharacter, songs[curSelected].songCharacter + "0", 24);
